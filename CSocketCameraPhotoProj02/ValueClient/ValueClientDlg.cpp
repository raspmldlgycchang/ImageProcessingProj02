...
BOOL CValueClientDlg::OnInitDialog()
{
	...
	m_client.Create();
	m_client.Connect(L"192.168.43.169", 26001);
...
void CValueClientDlg::OnBnClickedSendBtn()
{
	CString str;
	CComboBox*p;
	p = (CComboBox*)GetDlgItem(IDC_OPT_COMBO);
	int index = p->GetCurSel();
	if (index != CB_ERR) {
		p->GetLBText(index, str);
		SetDlgItemText(IDC_OPT_EDIT, str);
		//wchar_t*와 CString은 캐스팅만으로 변환가능(길이변화 없음)
		wchar_t* unicode = (wchar_t*)str.GetBuffer(str.GetLength());
		int len = WideCharToMultiByte(CP_ACP, 0, unicode, -1, NULL, 0, NULL, NULL);
		char *p = new char[len];
		WideCharToMultiByte(CP_ACP, 0, unicode, -1, p, len, NULL, NULL);
		m_client.Send(p, len);
		delete[] p;
	}
